...
export default function RiskAssessment() {
  ...
  useEffect(() => {
    // Example call to backend AI prediction API
      ...
      try {
        const response = await fetch('/api/predict-risk', {
          ...
          body: JSON.stringify({
            ...
            formFieldsCount: 6, // example value
            unnecessaryFieldsCount: 3, // example value
            privacyPolicyUrl: 'https://example.com/privacy' // example URL
          })
        });
        const data = await response.json();
        setAiPrediction(data);

        // Add AI prediction as a new risk factor
        setRiskFactors(prev => [
          ...prev,
          {
            id: "ai-prediction",
            name: "AI Data Exposure Risk",
            score: data.riskScore,
            maxScore: 100,
            description: `AI prediction risk level: ${data.riskLevel}. Reasons: ${data.reasons.join(', ')}`,
            improvement: "Review flagged issues and take necessary actions.",
            fixed: false
          }
        ]);
        setRadarData(createRadarData([
        ...
        toast({
          title: "Error fetching AI prediction",
